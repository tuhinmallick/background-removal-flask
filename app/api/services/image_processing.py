...
class U2NetProcessor:
    def __init__(self, portrait_model_path, human_seg_model_path):
        self.portrait_net = self._load_model(portrait_model_path)
        self.human_seg_net = self._load_model(human_seg_model_path)
    ...
    def inference(self, net,input):
        ...
        return pred
    def process_portrait_image(self, input_img):
        face_cascade = cv2.CascadeClassifier('./saved_models/haarcascade_frontalface_default.xml')
        face = self.detect_single_face(face_cascade, input_img)
        im_face = self.crop_face(input_img, face)
        im_portrait = self.inference(self.portrait_net, im_face)
        return (im_portrait * 255).astype(np.uint8)

    def process_human_segmentation(self, input_img):
        # Here, you will implement processing specific to human segmentation using self.human_seg_net.
        # For instance:
        face_cascade = cv2.CascadeClassifier('./saved_models/haarcascade_frontalface_default.xml')
        face = self.detect_single_face(face_cascade, input_img)
        im_face = self.crop_face(input_img, face)
        im_segmented = self.inference(self.human_seg_net, im_face)
        return (im_segmented * 255).astype(np.uint8)


processor = U2NetProcessor('./saved_models/u2net_portrait.pth', './saved_models/u2net_human_seg.pth')